...
本書は`dm-converter`の利用者と、生成されたGeoPackage・MapLibre出力を利用する
システム向けの仕様です。CLIの入力・出力・ログ・終了条件と、出力データの形式を
定義します。処理内容の説明を含みますが、プログラム内部のファイル構成や実装責務を
定義する資料ではありません。
...
| `LEVEL` | INTEGER | 任意 | 地図情報レベル |
| `DMREGION` | INTEGER | 任意 | DM要素レコードの地域分類コード |
| `DMINFO` | INTEGER | 任意 | DM要素レコードの情報分類コード |
| `DMELEMID` | INTEGER | 任意 | DM要素レコードの要素識別番号 |
...
| `DMATTRKIND` | INTEGER | 任意 | DM要素レコードの属性区分 |
| `DMUPYYMM` | INTEGER | 任意 | DM要素レコードの更新の取得年月 |
| `DMDELYYMM` | INTEGER | 任意 | DM要素レコードの消去年月 |
| `DMATTRDATA` | TEXT | 任意 | DM属性レコードの属性データ |
...
たとえば`DMCODE=7212`は、線進行方向の右側に並ぶ幅1.5mm・高さ0.75mmの
円弧と、端点間中央の0.5mm短線を補助線として生成します。
...
- グリッド、不整三角網、属性詳細レコードは地物として出力しません。検出時は
  警告として集計し、該当レコードをスキップします。
- MapLibre出力では、内周を持つ面を変換できません。
- 未実装の補助図形・記号展開は生成しません。
